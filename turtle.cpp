...
Turtle::Turtle(int xCoord, int yCoord) {
	symbol = 'Z';
	strength = 2;
	initiative = 1;
	x = xCoord;
...
void Turtle::action(vector<Entity*>& entities, World* world, Logger &log, int width, int height) {
...
int Turtle::collision(Entity* attacker) {
	if (attacker != this) {
...
void Turtle::makeChild(vector<Entity*>& entities, int x, int y) {
